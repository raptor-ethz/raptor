...
class VideoSender(Node):
    def __init__(self):
        ...
        self.color_pub = self.create_publisher(Image, 'color_images', 10)
        self.depth_pub = self.create_publisher(Image, 'depth_images', 10)

        self.cam = RSCamera()

        while rclpy.ok():
            color, depth = self.cam.get_raw_color_aligned_frames()
