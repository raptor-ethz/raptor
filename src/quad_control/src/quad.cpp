...
void Quad::goToPos(
  ...
  RCLCPP_INFO(this->get_logger(),
              "Received GoToPos request: [%f, %f, %f, %f, %f]", request->x_ref,
              request->y_ref, request->z_ref, request->yaw_ref, request->accuracy);
  ...
  float deviation = pow(telemetry_->position_velocity_ned().position.north_m - pos_msg.north_m, 2) + 
                  pow(telemetry_->position_velocity_ned().position.east_m - pos_msg.east_m, 2) +
                  pow(telemetry_->position_velocity_ned().position.down_m - pos_msg.down_m, 2);

  while(deviation > pow(accuracy, 2)){
    deviation = pow(telemetry_->position_velocity_ned().position.north_m - pos_msg.north_m, 2) + 
                pow(telemetry_->position_velocity_ned().position.east_m - pos_msg.east_m, 2) +
                pow(telemetry_->position_velocity_ned().position.down_m - pos_msg.down_m, 2);
  }

  RCLCPP_INFO(this->get_logger(), "Reached Position. Deviation: %f", deviation);
