...
const uint8 NumChannels = 4;  //For this example to work proper,
                              //SamplesPerPacket needs to be a multiple of
                              //NumChannels.
const uint8 SamplesPerPacket = 4;  //Needs to be 25 to read multiple StreamData
...
int main(int argc, char **argv)
{
    ...
    if( ConfigIO(hDevice, &dac1Enabled) != 0 )
        goto close;
    ...
    StreamData(hDevice, &caliInfo, dac1Enabled);
    StreamStop(hDevice);
